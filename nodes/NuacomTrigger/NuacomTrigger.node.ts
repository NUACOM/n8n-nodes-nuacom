	...
	description: INodeTypeDescription = {
		...
		properties: [
			...
			// Call filters
			{
				...
				type: 'options',
				default: '',
				description: 'Only trigger for calls in this direction. Leave as "Any" to receive all. Filters combine: the event must match every filter set.',
			...
			},
			{
				...
				type: 'options',
				default: '',
				description: 'Filters combine: the event must match every filter set — e.g. Queue + Extension only triggers for calls in that queue handled by that extension. Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a>.',
			...
			},
			{
				...
				type: 'options',
				default: '',
				description: 'Matches the extension that handled the call (answered, initiated, or on the call channel). Filters combine: the event must match every filter set — e.g. Queue + Extension only triggers for calls in that queue handled by that extension. Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a>.',
